...
public class cubeAuto extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        robot.ADM.setTargetPosition(25);
        robot.encoderDriveStraight(10, 2.0, opModeIsActive(), runtime);

        if(maxIndex == 0) {
            robot.turnByGyro(25, .3, opModeIsActive());
            robot.encoderDriveStraight(24, 2.0, opModeIsActive(), runtime);
            robot.brake();
            robot.turnByGyro(0, .3, opModeIsActive());
            robot.encoderDriveStraight(27, 2.0, opModeIsActive(), runtime);
            robot.marker.setPosition(robot.markerIn);
            sleep(1500);
            robot.marker.setPosition(robot.markerOut);
            robot.brake();
            robot.turnByGyro(40, .3, opModeIsActive());
            robot.encoderDriveStraight(-90, 10.0, opModeIsActive(), runtime);
        }
        if(maxIndex == 1) {
            //Do Not Turn
        }
        if(maxIndex == 2) {
            robot.turnByGyro(-30, .5, opModeIsActive());
        }
